...
//Read and Write completion sephamore signals
OS_ECB *I2C_Write_Sephamore; /*!< Signals successful write completion */
OS_ECB *I2C_Read_Sephamore; /*!< Signals successful read completion */
enum
{
  STATE_IDLE,
  STATE_DEVADDRESS,
...
bool I2C_Init(const uint32_t moduleClk, const TI2CModule* const aI2CModule)
{
  ...
  //turn call back functions into threads
...
//  I2CModule.readCompleteCallbackArguments = aI2CModule->readCompleteCallbackArguments;
  ...
  //Initialise new write and read sephamores
  I2C_Write_Sephamore = OS_SemaphoreCreate(0);
  I2C_Read_Sephamore = OS_SemaphoreCreate(0);
...
{
  //Wait until other I2C write processes end
  OS_SemaphoreWait(I2C_Write_Sephamore,0);
  ...
  //Tell next I2C write process to begin
  OS_SemaphoreSignal(I2C_Write_Sephamore);
}
...
{
  OS_SemaphoreWait(I2C_Read_Sephamore,0);
...
//  }
  ...
  //  The I2C_S_BUSY_MASK is used to determine whether the I2C bus is idle.
  //  You should check this before you initiate a communication.
  I2CWaitIdle(); //wait until I2C is idle
  //send start signal
  I2CStartSignal();
  ...
  OS_SephamoreSignal(I2C_Write_Sephamore);
