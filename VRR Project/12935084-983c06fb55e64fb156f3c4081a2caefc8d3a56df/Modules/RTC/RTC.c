...
OS_ECB *RTC_Update_Semaphore; /*!< signals OS that the clock has changed*/
...
//  UserArguments = userArguments;
  ...
  //Initialise new sephamore to 0
  RTC_Update_Semaphore = OS_SemaphoreCreate(0);
...
/*! @brief RTC interrupt request handler signals OS that clock value has changed
 *
 */
void RTC_Seconds_IRQHandler(void)
{
  OS_ISREnter();
  OS_SemaphoreSignal(RTC_Update_Semaphore); //Signal to the OS that the clock has changed
  OS_ISRExit();
}

/*! @brief Manages Real time clock value thread
 *
